...
$(document).ready(function () {
  const swiper = new Swiper('.banner-swiper', {
    ...
    // Если надо, можно добавить автопрокрутку:
    // autoplay: {
    //   delay: 5000,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tabs = document.querySelectorAll('.section-steps .tab');
  const slider = document.querySelector('.section-steps-slider');
  ...
  tabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      slider.setAttribute('data-type', target);

      // Здесь можно переключить контент для разных табов, если нужно
    });
  });
});
